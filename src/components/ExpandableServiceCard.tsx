...
const ExpandableServiceCard = ({ title, category, location, description, image, fullDescription, allProjects = [], onCategoryClick }: ServiceCardProps) => {
  ...
  const shareOptions = [
    { 
      ...
      action: () => window.open(`https://wa.me/?text=${encodedMessage}%0A%0A${encodeURIComponent(shareUrl)}`, '_blank') 
    },
    { 
      name: 'Facebook', 
      icon: (
      ...
      action: () => window.open(`https://www.facebook.com/sharer/sharer.php?u=${encodeURIComponent(shareUrl)}&quote=${encodedMessage}`, '_blank') 
    },
    { 
      name: 'Instagram', 
      icon: (
    ...
    { 
      ...
      action: () => window.open(`https://twitter.com/intent/tweet?text=${encodedMessage}&url=${encodeURIComponent(shareUrl)}`, '_blank') 
    },
    { 
      name: 'LinkedIn', 
      icon: (
    ...
    { 
      name: 'Email', 
      icon: <Mail size={20} />, 
      action: () => window.location.href = `mailto:?subject=CLAUSE INTERIOR FITS - ${encodeURIComponent(title)}&body=${encodedMessage}%0A%0A${encodeURIComponent(shareUrl)}` 
    },
    { 
      name: 'Copy Link', 
      icon: (
